...
export async function sendToAllUsers(results: UserMatchResult[]): Promise<void> {
  ...
  for (const result of results) {
    try {
      // Filter out jobs shown to this user in last 14 days
      // ✅ MAXIMUM FILTERING: Block jobs for 2 weeks (user requirement)
      const freshJobs = cache.filterRecentlyShown(
        result.matched_jobs,
        result.username,
        14 // Don't show same job within 14 days (maximum filtering)
      );

      const stats = cache.getFilterStats(result.matched_jobs, result.username, 14);
      logger.info(
